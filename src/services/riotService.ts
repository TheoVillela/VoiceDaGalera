...
import { api, MatchResponse, PuuidResponse, SERVER_ATIVO } from './api';
...
export const getPuuid2 = async (
  ...
  const res = await fetch(`${SERVER_ATIVO}/getUserId?summonerName=${summonerName}&tagLine=${tagLine}`);
...
export const getPartida2 = async (
  ...
  const res = await fetch(`${SERVER_ATIVO}/getActiveGame/?puuid=${puuid}`);
